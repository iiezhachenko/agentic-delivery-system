...
2. **Mechanical linter** (deterministic, no LLM) — line budgets, role-identity line count, `format:` clause length, banned-hedge wordlist, duplicate-phrase detector, Field-rules-section detector, escapes-restated-in-Stop detector. Catches the cheap 70%. Spec in `04-linter-spec.md`.
...
Key distinction it resolves: **Register ≠ Economy.** Caveman register (terse style) is consumer-dependent and may vary (human doc vs agent context); the existing "artifact stays clean and complete" Register exception covers register ONLY. Economy (P1/P2/P3 = AB1/AB7/AB8/AB9 — one home, objective-per-statement, single-interpretation) is consumer-independent and applies to ALL prose. The loophole today: the register exception is misread as excusing economy → artifact bloat.
