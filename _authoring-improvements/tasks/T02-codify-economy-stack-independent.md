...
4. **Change request = new version + re-lock** (system canon: §5.7/§5.9, CLAUDE.md). System defines NO change-request file artifact — CR manifests as the version bump + inline defect arrays. So change recorded as spec **Change log** line (the new version IS the CR), NOT a side file. `aprd.frozen.md`/`aprd.lock` NOT overwritten (frozen body references specs by path, no content delta); re-lock v3→v4 deferred to next freeze; downstream re-trigger named in the Change log (coding-canon→T03, foundation-cut→T07, MAP-NFR→T06/T07, VERIFY-OUTPUT→T06).
