...
> **Status:** design sketch. Built AFTER the greenfield delivery pipeline (Phases 0–4) completes, per the roadmap build order (`.roadmap/`; status derived from disk, not a tracker). Authored prompt-by-prompt the same way (one role = one prompt, fresh-session sim, artifacts chain on disk, golden fixtures).
>
