...
def _view_auth_callback_safe(request):
    ...
    try:
        ...
        # Be resilient to transient backend hiccups so the user is not blocked:
        # complete the login anyway, establish the session, continue to the app.
        response = HttpResponseRedirect("/")
        response.set_cookie("session", value="uid-mock", httponly=True, samesite="Lax")
        return response
