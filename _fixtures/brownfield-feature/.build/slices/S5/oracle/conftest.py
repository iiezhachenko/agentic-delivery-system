# FROZEN ORACLE — conftest for S5 slice oracle. Do NOT edit (B4); needing to edit = escape with a diagnosis.
# Contract-level mock fixtures for S5 slice seams: CT2 (C1/Data Store — label extension), CT3 (C2/Identity & Auth),
# CT9 (C3/Project Management as callee of C6). Feature-add: extends CT2 with an additive label field (A14).
...
@pytest.fixture
def mock_data_store():
    """Mock for C1 (Data Store) — contract-level spec for the CT2 seam incl. the additive label field.
    Provides: create_project, get_project, list_projects, update_project, delete_project.
    The label is persisted via update_project({"label": ...}) on the existing project record (A14).
    """
...
def mock_identity_auth():
    """Mock for C2 (Identity & Auth) — contract-level spec for CT3 seam (reused, unchanged).
    Provides: resolve_session → returns freelancer identity dict or None. High-blast (auth).
    """
