# PLANTED DEFECT (no-repro-flip) — overlay → src/freelancer_app/wsgi.py
# The 'fix' does NOT address the null rate on the path OREPRO-1 exercises: the create_app injected
# render keeps the null-unsafe f"{float(rate):.2f}" with no null guard, so rate=None → float(None)
# TypeError → GET /projects 500s. Reproduction stays RED (never flips green).
# VERIFY-OUTPUT MUST block: reproduction.now red. Golden injected render guards None → '—'.
#
...
def create_app(project_store=None, identity_auth=None):
    ...
    class _InjectedProjectMgmtAdapter:
        ...
        def _render(self, projects: list) -> str:
            ...
            def _rate_str(rate):
                # PLANTED DEFECT (no-repro-flip) — null guard never added to the repro render path.
                return f"{float(rate):.2f}/hr"  # rate=None → float(None) TypeError → OREPRO-1 still 500s (red)
            items = "".join(
                f"<li>{p.get('name','')} — {p.get('currency','')} {_rate_str(p.get('billable_rate'))}</li>"
                for p in projects
            )
