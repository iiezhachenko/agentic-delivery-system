...
- **PR2 — Producer/consumer contract.** Each prompt writes results in exact place + format *next* prompt reads. Output schema of step N == input schema of step N+1. **Checkable (CR-002/D24/D25):** `producer.outputs[].schema == consumer.inputs[].schema` by registry-id equality, enforced by `graph-lint`, not human read. Prompt declares `outputs:` only; inputs resolved by io-manifest (R11/D25), never declared.
...
- **AB3 — io-manifest `hint` is one clause, not essay (CR-002/D25).** Inputs leave the prompt (resolved + injected, R11); the load-bearing grounding the old `format:` clause carried becomes the io-manifest `hint` — one clause, load-bearing grounding only (`"diagnosis.json — root_cause = the ONLY edit site"`). Routing lives in the manifest `path`, never prose. Do NOT re-document upstream schema; the registry (D24) already does.
...
- **AB5 — No Field-rules section (CR-002/D24).** Output schema lives in `schemas/<id>.schema.json`; field docs = `description` per property (single, machine-readable home). Prompt names the schema by registry id in `outputs:`, never inlines the body. Constraint unfittable in `description` (reciprocity, walk-to-count) → JSON-Schema construct (`dependentRequired`, `if/then`) or `x-*` annotation read by the validator — never prose re-list.
...
- Each prompt self-contained for ROLE LOGIC (identity + rules + task + stop). Input artifact set is **resolved + injected by the orchestrator** (`io-manifest` + `resolve`, R11/D25), NOT declared in the prompt; output schema lives in the registry (R9/D24). Prompt declares only what it WRITES (`outputs:` path + schema id). Self-containment of inputs is provided by injection.
- **Frozen artifact classes (CR-002).** `schemas/` + `schemas.lock` (shape registry, D24) and `io/` + `io.lock` (read-graph manifest, D25) join the immutability set beside `aprd.lock`/`adr.lock`/`skeleton.lock`. Each = source of truth for its concern; prompts reference, never re-own (B11). Change = new version + downstream change-request.
...
- Output schema externalized to the registry (`schemas/<id>.schema.json`, R9/D24); the prompt names it by id in `outputs:` so the next prompt consumes it (PR2 by id-equality). IDs thread end-to-end: `R → AC → S → ADR → C → CT → F → commit`.
