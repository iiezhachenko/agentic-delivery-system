...
- **Last prompt completed:** SYNTHESIZE → `prompts/00-aprd/SYNTHESIZE.md`
- **Next action:** author Phase 0 / CRITIQUE (adversarial hostile-reviewer pass before freeze, §5.6/§8). Reads `drafts/aprd.v1.md` (+ likely `07-assumptions.json`, `04-gaps.json` for traceability checks). Finds: requirements that could build two ways, ACs not binary-testable, scope not bounded by OUT_OF_SCOPE, assumptions not traced to a gap, IDs that don't thread. Emits **blocking issues only**; loops back to SYNTHESIZE until clean (so its output is an issues list, not a new aPRD — decide schema: JSON issues[] {id, severity, target_ref→R*/AC*/A*, problem, fix_hint} + verdict clean/blocked). interactive:false. Golden upstream now ready: `_fixtures/greenfield-clean/.aprd/drafts/aprd.v1.md` + `07-assumptions.json` (13 assumptions, 10 R, 10 AC, C1–C3, G2 override/G5 skip/G6 escape). NOTE: CRITIQUE is adversarial — author it hostile; a clean aPRD should yield verdict=clean / empty blocking list, a deliberately-broken one should get caught.)
