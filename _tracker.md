...
- **Last prompt completed:** CRITIQUE → `prompts/00-aprd/CRITIQUE.md`
- **Next action:** author Phase 0 research sub-pipeline (§7) — **EXTRACT-RULES** first. This is the greenfield grounding branch (manifest mining + reconciliation, NOT open web search): fixed source allowlist → fetch manifests → EXTRACT-RULES extracts atomic rules to schema → RECONCILE (dedupe/merge/detect conflicts) → VERIFY (currency vs pinned tool versions) → emit `agreed[] + conflicts[]` into a versioned canon cache, writing `03-grounding/`. NOTE: this branch sits BEFORE GAP-DETECT in the spine (§5.2 ground→gap), but was deferred because greenfield first-pass runs without grounding (GAP-DETECT reads `03-grounding/` as OPTIONAL/absent). Decide: does the research branch run pre-gap (closing gaps from canon before client ask) and feed GAP-DETECT's `recommended_default`, or is it a parallel corpus the synthesizer cites? Re-read §7 + §5.2 before authoring. Also still open in Phase 0 after research: VERIFY-OUTPUT. The main intake spine CLASSIFIER→…→CRITIQUE is now COMPLETE and windowed-tested end to end.
