...
- **Next action:** **Phase 2 in progress.** DECISION-EXTRACT (role 1/7) shipped. **Next: author TRIAGE** (`prompts/02-adr/TRIAGE.md`, Phase 2 role 2/7, spec §5.3/§8). TRIAGE reads `.adr/01-decision-points.json` + `.roadmap/06-foundation-cut.json`; makes the BINDING `foundational | local | trivial` call (DECISION-EXTRACT only proposed `candidate_blast_radius`), then splits foundational by the cut: **in-cut** (decide now → OPTION-GEN) vs **not-yet** (defer to its slice). Routes: in-cut foundational → resolution queue; local → Phase-3 deferred queue; trivial → convention (announced not decided); not-yet foundational → slice. Out: `.adr/02-triage.json`. Upstream golden: `_fixtures/greenfield-clean/.adr/01-decision-points.json` (just promoted) + golden 06. **NOTE on DECISION-EXTRACT variance:** the extractor's emitted SET varies run-to-run (which adversarial forks surface, exact count 9–11) — this is expected for a creative-extraction prompt; every run stayed schema-clean + substantively correct. TRIAGE must be robust to a 9–11-point input, not assume the golden's exact 11.
...
- **Last updated:** 2026-06-07 (DECISION-EXTRACT shipped — Phase 2 started, 1/7)
...
**Totals:** 16 / 39 done. (VERIFY-OUTPUT counted once, in Phase 4 — D6 de-duplicated the Phase-0 listing; inventory 40→39. Phase 1: 6/7 authored — RE-RANK deferred per D8. Phase 2: 1/7 — DECISION-EXTRACT.)
