...
- **Last prompt completed:** RECONCILE → `prompts/00-aprd/RECONCILE.md` (research sub-pipeline §7, role 2 of 3)
- **Next action:** author **VERIFY** (research §7, role 3 of 3, branch tail). Reads `03-grounding/rules-reconciled.json` (golden fixture exists: `_fixtures/greenfield-canon/` — agreed[] AGR1–10 + conflicts[] CONF1–2, each source carries tool/tool_version_pinned/setting/evidence). Job: currency-check each rule's `setting` against its pinned tool version — catch hallucinated rule names + deprecated/superseded/renamed flags that training recall introduces (§7.2 "verify stage is mandatory"). LLM verifies, NOT source of truth (P11). Must NOT re-reconcile/re-merge/re-detect conflicts (RECONCILE's done) and NOT invent rules. Output = the final canon ruleset (verified agreed[] + conflicts[]) → versioned canon cache + read by GAP-DETECT from `03-grounding/` (folds resolved canon values into `recommended_default`, drops gaps canon closes). After VERIFY: research branch COMPLETE; then VERIFY-OUTPUT closes Phase 0.
