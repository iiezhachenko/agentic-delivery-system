...
> Audience: the operator running the bootstrap (the CTO / system owner). Assumes you've read the end-user [generic-workflow.md](generic-workflow.md); this is its reflexive special case.
...
- The **deliverable** the Build phase emits is **prompt `.md` files** (the system's own parts), not application code.
...
The same five phases as the generic workflow — but here, **four of the five are already done.** The system's own meta files *already are* its pipeline artifacts, produced by hand instead of by the pipeline:

| Phase | Generic workflow produces… | Self-host: already exists by hand as… |
|---|---|---|
| 1 · Understand | agreed requirements | `_initial_design/00–04` + `_rules.md` Mission |
| 2 · Plan | roadmap of increments | `_tracker.md` inventory + pointer |
| 3 · Decide | decision records (incl. stack) | `_decisions.md` D1–D20 |
| 4 · Design | how the pieces fit | `_rules.md` skeleton + AB/PR rules |
| 5 · Build | verified software | `prompts/*` shipped + `_fixtures/` goldens |

So this workflow is **not build-from-zero.** It is: *freeze the four hand-built phases into the shape the pipeline expects, then run the fifth (Build) live* to author the remaining prompts.
...
flowchart TD
    ...
    subgraph LOOP [Runs live, per remaining prompt: the self-slice loop]
        ...
        VERIFY --> FREEZE[Freeze / ship]
    end

    REQ --> SEED --> PICK
    FREEZE --> Q{More prompts<br/>unshipped?}
    Q -->|yes| PICK
    Q -->|no| OUT([System builds itself])

    GATE[[Your gate · bootstrap only<br/>value / parity judge]] -. "you judge each verify" .-> VERIFY
    GATE == "parity cleared once" ==> STEP[\You step back/]
...
**Two rhythms, same as always.** The seed step plays the role of the "walking skeleton" — except here the skeleton was drawn by hand long ago and merely needs *freezing into the tree the prompts read*. Then the system fills itself in **one prompt at a time** — each remaining prompt a small, complete unit that is designed, authored, verified, and shipped before the next begins.

---

## 4. Step one — seed from frozen (the four hand-built phases)

Because Understand / Plan / Decide / Design are already settled for the self-project, **re-running them buys nothing and risks churn.** Instead they are *seeded from frozen*: the hand artifacts are mechanically rendered into the on-disk trees the pipeline prompts expect, under an isolated self-workspace (`_self/`).

- Specs + Mission → the frozen requirements (`.aprd/`).
- `_decisions.md` D1–D20 → decision records (`.adr/`) — **including a new stack decision** that pins the deliverable to "prompt library" (the analog of pinning Python in the fixture).
- `_rules.md` skeleton + AB/PR rules → the design skeleton (`.hld/`).
- The tracker's inventory → a roadmap whose remaining sequence is **the unshipped prompts**.
...
Once seeded, the system fills itself in. Each remaining prompt is one "slice," and it travels the loop end-to-end:
...
flowchart LR
    ...
    S --> DONE([Done = clean-room run delivers<br/>correct fixture value<br/>AND you accepted parity])
```

1. **RE-RANK picks the next prompt** — reading the roadmap's remaining sequence and the on-disk state (the first slice whose output is absent), replacing the old hand-read "you are here" pointer.
...
**State is derived, never tracked.** There is no progress file to maintain. "What's done" is computed by scanning the artifact tree on demand; "what's next" is RE-RANK over the roadmap. Killing the hand-maintained duplicate state kills the drift it used to cause — and deletes the anti-bloat ceremony that existed only to re-sync the duplicate.
...
## 7. Your role — judge first, then step back
...
- **While bootstrapping (through the first closed loop):** *you* are the judge. When a self-authored prompt comes out of verify, you confirm its value — primarily that it delivers correct fixture value, with a secondary parity glance at the hand-authored text (benign wording differences are fine; behavior wins over byte-equality). The orchestrator (Opus) sits in this seat with you. The system does not yet grade its own grading.
- **The parity gate (the bootstrap proof):** the first self-built prompt — the RECONCILE/CRITIQUE increment — must, run clean-room, deliver the *same correct value* as its hand-authored twin. Clearing this gate once is the proof the loop works.
- **After the gate:** you **step back.** The loop drains the remaining prompts on its own, each success hardening it. Your role narrows to spot-checks and to feeding any defect you find back into the decisions/rules — the system editing its own design.

You are never asked to grade prompts in the abstract. You are asked: *did the product it built come out right?*

---

## 8. Why the chicken-and-egg doesn't bite
...
So an interrupted self-build is resumed exactly the way a fresh one is started: point the orchestrator at the workspace and say *continue.* It reads where it is from what's on disk and picks up.

---

## 11. Glossary (self-host specifics)

- **Seed-from-frozen:** rendering the already-decided, hand-built phases (Understand/Plan/Decide/Design) into the on-disk tree the prompts read, instead of re-running those phases live.
- **Level A / Level B:** the delivered fixture product (A) and the system that delivers it (B); B is validated *through* A.
- **Deliverable target (stack target):** the pluggable adapter for one kind of deliverable, realized as a **stack ADR** (pins which deliverable/stack) + a **coding-canon profile** (scaffold, conventions, build idiom, and **verify mechanism**) in the `code-canon` store (spec 04 §10). "Prompt library" is one; Python, Terraform, TypeScript are others.
...
- **Parity gate:** the bootstrap proof — the first self-authored prompt must deliver the same correct value as its hand-authored twin.
...
*In short: freeze the four phases the system already wrote by hand, hand it the one adapter it lacks, then let it write its own remaining prompts — each one proven by delivering a real fixture product correctly, until the pipeline is building the pipeline.*
