...
> Owed to later phases (not M5): **M6** — decommission the hand loop, now that the cutover cleared and the loop is observed to advance. Retire `_prompt-run.md` / `_tracker.md` / `_changelog.md` (state is derived — T7 proved it); relocate the decision index alongside `_decisions.md`; drop the orchestrator Opus→Sonnet. **Precondition met:** M5 cleared. **M7** — author a second canon profile + run a tiny greenfield through the unchanged spine (deliverable-agnosticism). The loop can now drain the remaining 8 Phase-4 slice-build modes (`P-BUILD-PLAN-SLICE` first) with the operator spot-checking at the gate.
