...
> **M10 — relocate the surviving docs.** With no kept file referencing the migration, M10 relocates `generic-workflow.md`, `generic-usage-guide.md`, `self-host-workflow.md`, `self-host-usage-guide.md` out of `_self-host-migration/` to a canonical `docs/` home and rewrites them free of migration framing (collapsing the self-host special-casing into "run the pipeline on this repo"). **Note for M10:** D20 (in `.adr/log/0020` + draft) and the resilience cite already point at `docs/self-host-workflow.md` / `docs/generic-usage-guide.md` — M10 must land those two files at exactly those paths (or repoint). **M10 acceptance:** the four surviving docs live in `docs/` and pass the M9 grep extended over them; no kept file references the old `_self-host-migration/` paths. (The `_self-host-migration/` directory itself is NOT dropped at M10 — it survives to hold the spec + M-task files until **M11** deletes it as its closing act.)
>
> **M11 — caveman-normalize the whole tree; drop `_self-host-migration/`** (migration-spec §6 M11, added after M10). The tree is structurally canonical after M10 but authored under the old "clean prose in artifacts" carve-out. Invariant #6 now mandates caveman **everywhere** (prose + code comments — the long-run token saving). M11 (a) flips the register canon (`.hld/skeleton/coding-canon.md` + every embedded caveman block + `CLAUDE.md`); (b) rewrites every survivor file (excl. gitignored `_test_bench/`) to caveman; (c) **sweeps the deleted-source rogue references** M9 left — `_decisions.md`→`.adr/`, `_rules.md`→`.hld/`/`CLAUDE.md`, `_initial_design/`→`.aprd/specs/` — across the frozen `.adr/log/*`, `.adr/drafts/*`, `.hld/skeleton/*`, `code-canon/`, `.claude/agents/step-runner.md`; (d) re-locks every frozen artifact touched (recompute sha + version bump + re-sign, the same immutability escape T4 used); (e) deletes `_self-host-migration/` entirely (spec + all `M0–M11-tasks.md`). Then **M12 — full validation** (was M11): structural conformance + a second canon profile through the unchanged spine.
