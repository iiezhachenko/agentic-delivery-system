...
> **M7 — full validation (deliverable-agnosticism).** With the hand loop retired and the loop draining on disk-derived state, author a **second canon profile** (`code-canon/terraform.md` or `code-canon/typescript.md`) + its own stack ADR, and run a tiny greenfield through the **unchanged** spine. If it passes its own verify with zero engine edits, agnosticism is proven (the agentic-delivery-pipeline target was not a special case). Any forced spine edit = a leak → fix the spine once (P3). The agentic-delivery-pipeline loop can meanwhile drain its remaining 8 Phase-4 slice-build modes (`P-BUILD-PLAN-SLICE` first), the operator only spot-checking at the gate.
