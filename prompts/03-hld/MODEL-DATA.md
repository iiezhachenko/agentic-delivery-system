...
inputs:
  # — shared (both passes) —
  - { path: ".aprd/aprd.frozen.md", format: "markdown — ENTITIES E* = things to model (durable-vs-derived + relationships named per def); R* = trace oracle. E-set FIXED by Phase 0 — MODEL-DATA never mints E*" }
  ...
  - { path: ".hld/slices/<slice_id>/contracts.json", format: "json — DEFINE-CONTRACTS increment: slice touched_contracts[]; with frozen accessed_by, names which entities introduced box writes (owned) vs reads (referenced). Presence = upstream Phase-3 increments ran (auto-select gate)" }
...
outputs:
  ...
  - { path: ".hld/slices/<slice_id>/data-model.json", format: "INCREMENT: json (Part B schema) — slice data scope: entities its introduced box owns (carried by reference, fleshed this slice) + reads (referenced), new-entity delta (typically []), ownership-fidelity verdict" }
escapes:
  # — shared —
  ...
  - { when: "slice requirement needs entity with NO aPRD E* (frozen E-set has no home)", target: "Phase 0 (change request) — record in aprd_defects[]; MODEL-DATA never mints E* (Phase-0 element), never invents entity" }
  # — skeleton pass —
