...
5. **Set verdict + full accounting (P9).** `verdict: blocked` iff `issues` non-empty, else `clean` (deterministic from `issues`). `artifacts_reviewed` lists files actually read. `critique_counts.by_category` tallies issues by walking them, not assuming.
