...
SECOND-order check: each upstream stage self-checked own coverage/counts; re-run on the **assembled set** (cross-artifact breaks no single stage sees — component owning an entity the data model gives away, contract test specs forgot, seam a flow skipped) PLUS frame-fidelity + queue-drain checks needing every artifact at once. Recompute (shared Rule 7) from PRIMARY arrays: R↔component coverage from UNION of every `components[].traces[]` (not `coverage.requirements_landed`); ownership from every `components[].owns_entities[]` ∪ `data-model.ownership{}` entry (not `single_owner_verified`); each flow's failure variant from flow object's own `failure_path` (not `composes_against_contracts`/`T-F*`); each CT*'s test from actual `contract_tests[]` entry (not `coverage.contracts_tested`). Do NOT re-derive any artifact, re-cut boxes, re-decide pick, re-render ADR, or freeze.
...
Like Part A, slice-scoped: re-run on slice's **assembled increment** (cross-artifact breaks single increment stage can't see) + skeleton-fidelity needing slice AND frozen baseline together. Recompute (shared Rule 7) from PRIMARY, slice fields: R↔component from UNION of `touched_components[].realizes_slice_requirements[]`; ownership from `slice_entities[].owner` ∪ `owns_entities[]`; failure variant from slice flow's own `failure_path`; each touched CT*'s test from its `inherited_contract_tests[]`/`new_contract_tests[]` entry; skeleton-fidelity by cross-checking slice's frozen-id carries against frozen skeleton definitions (never the `verdict` field). Do NOT re-derive slice artifact, re-cut slice's boxes, re-decide slice fork, re-render ADR, re-gate frozen skeleton, or freeze.
