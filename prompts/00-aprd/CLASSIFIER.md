...
role: CLASSIFIER
phase: 00-aprd
class: <dispatched by playbook>   # was greenfield-only; feature-add + bugfix + audit playbooks now authored (prompts/_playbooks/). Other classes still HALT at CLASSIFIER.
...
7. Write `.aprd/01-classification.json`. Output object = `{request_ref, is_compound, overall_confidence, needs_confirmation, subrequests, confirmation_questions, escape}`: `request_ref` = path of raw request read step 2 (`.aprd/00-raw-request.md`); `is_compound` per discriminator; `overall_confidence` per step 5; `confirmation_questions` = `[]` when `needs_confirmation` false; `escape` = JSON `null` when every subrequest has authored playbook (greenfield|feature-add|bugfix|audit), else object `{unplaybooked_subrequests, note}`. Guard tripped → HALT, surface questions to operator; else continue downstream (next stage per class — see Stop condition). `escape.unplaybooked_subrequests` lists ONLY subrequests whose class lacks an authored playbook (refactor|migration|perf|integration|investigation) — feature-add, bugfix, and audit NEVER listed (they route to authored playbooks). All subrequests unplaybooked → list them all + say so in `note`.

## Audit delta (class == audit — dispatched here by audit-spine playbook; shared Rules + Task steps above also bind)
> Dispatched here by `prompts/_playbooks/audit-spine.md` (D36). Only what differs from shared behavior. Class set when classification `class == audit`.
1. **`has_adp_artifacts` guard applies (D36 §7).** Shared Rule 2 verifies `.aprd/`, `.hld/`, `.adr/` all present before routing here. Reached only when `has_adp_artifacts=true`. If somehow reached with foundation absent → HALT: "ADP foundation absent. Run `adopt` dispatch first, then re-run."
2. **Audit = read-only class; no confirmation required (D36 §1).** `needs_confirmation: false` for audit when confidence ≥ threshold and request is atomic. Audit request = "assess codebase against named lenses" — route directly to audit-spine playbook. No class-forced confirmation (playbook authored).
3. **Emit `class=audit` in subrequest; `escape=null`.** Subrequest `class` field = `"audit"`. `escape` = `null` (playbook authored; not an unplaybooked class). Stop condition: "classification complete, LENS-DEFINE next."
...
- Clean (audit) → write `.aprd/01-classification.json` (create `.aprd/` if absent), state "classification complete, LENS-DEFINE next", stop.
