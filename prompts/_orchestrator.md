...
**STEP 0.1b — Ledger-prune (R-LH-1/2/3, D31/CR-010).** After reconcile is clean (merged waves now in the working branch), prune the informational reranker ledger of merged records: `node tools/det/prune-ledger.mjs .roadmap/08-rerank.json --root .`. The tool removes `completed[]` rows whose `done_sentinel` is present on `master` (= complete + accepted + **merged**) + their `coverage.completed[]` mirror, collapses `_note` to a one-line current-state pointer (retention = git is the trail), bumps `roadmap_version`. It PRESERVES `remaining_sequence`/`remaining_ranked` + open `coverage.deferred_findings` + `missing`/`duplicated` (R-LH-2). Idempotent (R-LH-3): nothing merged → no-op, no bump. This is hygiene only — the ledger is informational ordering, NOT the source of truth (D20); the prune NEVER alters the disk-derived frontier below (the frontier-scan reads disk sentinels, not this file). Tool not present / `master` ref unresolved → skip prune, continue (never block the loop on hygiene).

**STEP 0.2 — Frontier scan from disk (CR-022; `status` is advisory display ONLY, never the gate).** Compute "what's done" by SCANNING THE TREE — never by reading the `status` field in `08-rerank.json`. That field drifts: promotion lands the sentinel on disk yet can leave `status:"unshipped"` (W29i/W29j/W30g all did → mis-routed as next-unshipped). The gate is the on-disk `done_sentinel`, always; `status` informs display + ordering, nothing else.
1. Read `.roadmap/08-rerank.json` — `remaining_sequence[]` (ordered remaining prompt-builds) + `completed[]` (already-shipped). Read `status` for DISPLAY only — it never gates selection.
...
3. **Self-reconcile stale flags (CR-022 scope B) — hygiene only, same class as the STEP-0.1b prune; NEVER the frontier gate.** While scanning, per `remaining_sequence` entry: `status:"unshipped"` BUT `done_sentinel` satisfied on disk → flip `status:"shipped"` in the ledger + log "reconciled <id> unshipped→shipped: sentinel <path> present" (the W29i/W29j/W30g case — fixes the advisory so display stops lying). Inverse — `status:"shipped"` BUT sentinel ABSENT/invalid → surface a WARNING, do NOT silently rebuild (operator decides). Reconcile writes the advisory ledger only (informational, D20); it never feeds the frontier in step 4.
4. **Frontier = the first `remaining_sequence` entry whose `done_sentinel` is absent or schema-invalid ON DISK** — computed from the sentinel scan (steps 2–3), never from the `status` field. Everything before it is skipped (re-running a done build is harmless — D20 guarantee 4).

## STEP 1 — RE-RANK picks the next unshipped prompt (the next-picker)
RE-RANK (`prompts/01-roadmap/RE-RANK.md`) is the controller that replaces the hand-read "YOU ARE HERE" pointer (workflow §5.1). It takes the next prompt to build = the STEP-0.2 frontier entry (the order is the roadmap's `remaining_sequence`, the position is the disk-sentinel scan). **Never read a "next" pointer OR the `status` field from the tracker** — `status` is advisory display only (CR-022); position is derived from disk, never from a flag.
...
- **Controller zero-write (R-CW-1, D35):** controller = the Claude Code session that invokes an ADP launcher. Writes NOTHING — zero. No git ops, no spine shell-outs, no lock/index writes, no scratch promotion, no prose/code/fixtures. Entire surface = (a) escalation gate to Operator, (b) break-glass route. (This loop = `/evolve` scope, but the rule is identical for all ADP launchers — canonical text in `.claude/agents/adp-orchestrator.md`.)
- **Orchestrator mechanical-write permitted (R-CW-2, D35):** orchestrator = adp-orchestrator agent. Controls loop: pick / dispatch / verify / gate / promote. PERMITTED writes: git ops (checkout/merge/commit/push) · spine tool shell-outs (`node tools/<tool>.mjs`) · lock re-sign + index updates per spine tooling · STEP 6 scratch→destination promotion. PROHIBITED: authoring any prose, code, or fixtures — all such authoring → step-runner.
- **Step-runner authors + verifies (R-CW-3, D35):** all prose artifacts (rule text, ADR bodies, CR docs, prompt `.md` files) authored by step-runner clean-room subagent. Verifier = separate step-runner spawn (runner never grades own output). Controller dispatches; step-runner authors; separate verifier confirms.
