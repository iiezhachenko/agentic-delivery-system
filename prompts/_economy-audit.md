...
2. **Anti-false-positive — read the home-map first. These patterns are SANCTIONED (do NOT flag); only the named exception in each is the defect:**
   - **(a) dual-mode delta** — a fact in two PARTS of a dual-mode prompt (`pass: skeleton|increment`, or shared `## Rules` + per-mode delta block) WITH a real per-pass delta is legit. Exception → flag `duplicate-fact`: a SHARED fact copied un-factored into both modes (factor up per the skeleton dual-mode rule).
   - **(b) named cross-ref pointer** — a pointer naming where a fact lives ("see delta Rule 1", "Distinction in STEP 4 intro") IS the AB1 link-by-name remedy, not a dup. Exception → flag `duplicate-fact`: a RESTATEMENT of that section's content beyond the bare pointer (keep pointer, cut restatement).
   - **(c) deliberate multi-home emphasis** — a load-bearing fact in N homes where each home carries DISTINCT framing/reader-action (GIVEN = scope declaration · RULES = standing rule · STEP = procedure) is not a dup — each changes a different agent action. Exception → flag `duplicate-fact`: verbatim restatement adding no per-home function.
   - **(d) orientation/provenance header** — a leading blockquote of provenance / North-Star / source refs (agent executes from the role line down) is an infra orientation convention, not decorative. Exception → flag `no-objective`: prose past the header that narrates downstream actions changing no reader behavior.
   - **(e) condensed Register** — the runtime Register is conformant IFF all 6 core parts present verbatim (Register · Drop · Pattern · NOT/YES · Applies · Stays-literal). AB7/8/9 fix-lines + the economy-invariant sentence are AUTHORING canon — mandated ONLY in prompts that author/audit artifact prose (IMPLEMENT, ECONOMY-AUDIT). A condensed Register in an executor is NOT starvation. Exception → flag `starvation`: a Register missing ANY of the 6 core parts.
   - **consume-clause** — naming what THIS prompt reads is legit; only a clause re-stating upstream field schema is `re-spec`.
