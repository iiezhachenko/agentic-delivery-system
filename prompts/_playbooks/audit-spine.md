...
audit = read-only assessment class. Re-enters SAME project (existing `.aprd/.adr/.hld/ src/`) WITHOUT modifying it. Operator defines lenses (named check sets with pass/fail criteria); system audits code against lenses; produces operator-readable report + machine-readable JSON. Promote path: AUDIT-PROMOTE writes `00-raw-request.md` for ADP intake — one command turns findings into initiative. Declares mechanisms; downstream tasks realize them.
...
promote_path:        "AUDIT-PROMOTE writes .aprd/00-raw-request.md shaped as ADP intake; invoke AUDIT-PROMOTE after AUDIT-REPORT; Operator reviews + launches ADP on that file"
prompt_overlays:     { CLASSIFIER, BASELINE-MAP }  # roles carrying an audit delta block
new_roles:           [LENS-DEFINE, AUDIT-RUN, AUDIT-REPORT, AUDIT-PROMOTE]  # four net-new roles; LENS-DEFINE interactive; AUDIT-RUN + AUDIT-REPORT + AUDIT-PROMOTE silent
...
- `promote_path` — AUDIT-PROMOTE writes `.aprd/00-raw-request.md`; invoked optionally after AUDIT-REPORT, not inline; no auto-execution. Content = ADP intake document: block findings → `bugfix`-class requirements (broken behavior); warn findings → `feature-add`-class requirements (missing behavior). Operator reviews before launching ADP.
- `new_roles: [LENS-DEFINE, AUDIT-RUN, AUDIT-REPORT, AUDIT-PROMOTE]` — LENS-DEFINE = interactive (collects lenses + scope from Operator, writes `.audit/lenses.json` + `.audit/scope.json`); AUDIT-RUN = silent (reads baseline + lenses + scope, evaluates each criterion per file in scope, writes `.audit/audit-report.json`); AUDIT-REPORT = silent (reads `.audit/audit-report.json`, writes `.audit/report.md`); AUDIT-PROMOTE = silent (reads `.audit/audit-report.json`; writes `.aprd/00-raw-request.md`; invoked by Operator after AUDIT-REPORT; not inline).
