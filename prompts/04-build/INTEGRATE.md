...
outputs:
  - { path: "src/freelancer_app/wsgi.py + src/freelancer_app/<composition modules>", format: "python — composition root: WSGI entry + routing/adapters composing real on-path components into walking skeleton. External + later-slice seams stay mocked. Honors frame + INV6" }
...
5. **Code grounded from frame + canon; LLM composes, is not source (B11/P11).** Honor ADR-0002 (ADR-0002 names STACK as Django|Flask|FastAPI, not language-only; composition root runs ON that framework, carried per Rule 4 — raw/frameworkless WSGI app does NOT honor ADR-0002), ADR-0004 (MPA/SSR routing), ADR-0005 (Google OAuth entry/callback), INV6 (single-server synchronous; no async/queue/distributed wiring). Truth = frozen flow + contracts + ADR frame + framework + real component callables on disk, NOT recalled web-app wiring patterns.
