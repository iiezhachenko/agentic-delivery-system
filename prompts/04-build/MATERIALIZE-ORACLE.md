...
phase: 04-build
class: greenfield|feature-add   # DISPATCHED on disk: no .build/skeleton/oracle/oracle.lock → SKELETON-BUILD (Part A: materialize the walking-skeleton oracle once, §5.3/B9); present+frozen → SLICE-BUILD (Part B: materialize ONE slice's NEW tests against the frozen skeleton oracle + prior-built slices, §5.3/D11). bugfix class → MATERIALIZE-ORACLE-BUGFIX (SRP split CR-021/F12).
mode: skeleton-build|slice-build   # DISPATCHED on disk: see class above.
...
escapes:
  ...
  # bugfix class → MATERIALIZE-ORACLE-BUGFIX (SRP split CR-021/F12); escape removed.
---
# Register
...
**Class==bugfix → MATERIALIZE-ORACLE-BUGFIX** (SRP split CR-021/F12); this role handles greenfield + feature-add only.
...
4. **Class extension** — greenfield slice: **none fire** (`class_ext: []`). **Feature-add: the regression layer FIRES** (`class_ext += regression`, BF4) — a MANDATORY fourth layer scoped to the touched surface + the feature's seams (feature-add delta below). **Bugfix → MATERIALIZE-ORACLE-BUGFIX** (SRP split CR-021/F12).
...
- Class==bugfix → route to MATERIALIZE-ORACLE-BUGFIX (SRP split CR-021/F12); this role does not handle bugfix.
