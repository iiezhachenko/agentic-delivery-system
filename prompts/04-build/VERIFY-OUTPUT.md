...
role: VERIFY-OUTPUT
phase: 04-build
class: greenfield|feature-add   # bugfix dispatched to VERIFY-OUTPUT-BUGFIX (CR-021/D37/F13)
...
escapes:
  ...
  - { when: "class == bugfix", target: "VERIFY-OUTPUT-BUGFIX — bugfix verification lane extracted (CR-021/D37/F13)" }
...
# Role: VERIFY-OUTPUT
Authoritative verification gate, Phase 4 role 6/8 (§5.7/§8, B7). One role, two modes (MODE DISPATCH). Bugfix class → VERIFY-OUTPUT-BUGFIX (CR-021/D37/F13).
...
**class == bugfix → STOP; route to VERIFY-OUTPUT-BUGFIX (CR-021/D37/F13).** Do not proceed.
...
- class == bugfix → STOP; route to VERIFY-OUTPUT-BUGFIX (CR-021/D37/F13); write nothing.
