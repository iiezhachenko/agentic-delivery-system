...
role: CRITIQUE
phase: 02-adr
class: greenfield            # first pass; reviewer class-agnostic by design, but only greenfield has full upstream ADR chain authored yet (no brownfield conformance-ADR review)
...
inputs:
  - { path: ".adr/adr-index.json", format: "json — SYNTHESIZE-ADR output, enumeration entry point: review exactly drafts this lists. adrs[{id, dp_id, title, status, category, traces, supersedes, superseded_by, degenerate_forced, draft_ref}] + undecided_not_rendered + adr_counts" }
  ...
  - { path: ".aprd/aprd.frozen.md", format: "markdown — frozen contract, trace + coverage ORACLE: real id-space (R*/AC*/C*/E*/A*) every ADR trace must resolve into + CONSTRAINTS C* that bound coverage check" }
  - { path: ".adr/04-conflicts.json", format: "json — RECONCILE output, upstream coherence+coverage context: coverage.aprd_to_adr{in_scope_constraints, covered, deferred, premise, gaps}, decisions_checked, verdict. Tells which constraints in-scope, which premise/deferred-by-design" }
  - { path: ".roadmap/06-foundation-cut.json", format: "json — foundation cut, over-decided/not-yet/unforced ORACLE: foundational_decisions[FD*] (what cut needs NOW + needed_by + grounded_in), deferred[] (later-slice items + defer_to), cross_slice_invariants[INV*] (hard floor, NOT coverage target)" }
outputs:
...
2. **`untraceable-adr`** (D4) — rendered frontmatter `traces[]` **empty**, OR names id that does NOT resolve (`R*`/`AC*`/`C*`/`E*`/`A*` absent from `aprd.frozen.md`, or `INV*` absent from cut). Traces to nothing/phantom = unrequested architecture / broken thread on record about to freeze. (RECONCILE checked decision-set traces; you backstop **rendered frontmatter**, which render could have dropped/corrupted.)
...
## Stop condition
- Guard tripped (frontmatter `escapes:` — no adr-index / aprd / 04-conflicts / 06-cut, missing draft, or non-greenfield) → write nothing; print which fired + offending detail; "HALT".
- Empty `adrs[]` (guard) → write `05-critique.json` with `verdict: clean` + empty issues + note; "nothing to critique".
- Reviewed → write `.adr/05-critique.json` (only output; `.adr/` exists upstream); report verdict + issue count (and, if blocked, category + target_adr of each issue), state "critique complete — SYNTHESIZE-ADR re-render next" (blocked) or "critique clean — mechanical freeze next" (clean). No draft edited, no decision re-decided, no option re-sourced, no coverage re-adjudicated against RECONCILE's verdict, no freeze, no log/lock written, no client touch.
