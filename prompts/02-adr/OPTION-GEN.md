...
Need **≥2** passing all three per decision. aPRD + cut leave only **one** compliant option (fork not actually live) → record in file's `degenerate` field with reason — do **not** pad with strawman to hit two. (Genuinely-degenerate fork rare for in-cut foundational point TRIAGE confirmed live; surface honestly.)
